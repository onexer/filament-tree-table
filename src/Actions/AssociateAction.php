...
class AssociateAction extends Action
{
    ...
    public function getRecordSelect(): Select
    {
        ...
        $getOptions = function (?string $search = null, ?array $searchColumns = []) use ($treeTable): array {
            ...
            $relationshipQuery
                ->whereDoesntHave($treeTable->getInverseRelationship(), function (Builder $query) use ($relationship): Builder {
        ...
        $select = Select::make('recordId')
            ...
            ->getOptionLabelUsing(fn($value): string => $this->getRecordTitle(Relation::noConstraints(fn() => $treeTable->getRelationship())->getQuery()->find($value)))
