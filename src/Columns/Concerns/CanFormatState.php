...
use Onexer\FilamentTreeTable\Columns\TextColumn;
...
trait CanFormatState
{
    ...
    {
        $format ??= TreeTable::$defaultDateDisplayFormat;
    ...
    {
        $format ??= TreeTable::$defaultDateTimeDisplayFormat;
    ...
    public function money(string|Closure|null $currency = null, int $divideBy = 0): static
    {
        $this->formatStateUsing(static function (TextColumn $column, $state) use ($currency, $divideBy): ?string {
            ...
            $currency = $column->evaluate($currency) ?? TreeTable::$defaultCurrency;
    ...
    {
        $format ??= TreeTable::$defaultTimeDisplayFormat;
