@php
    use function Filament\Support\get_color_css_variables;$state = $getState();
@endphp

<div
    wire:key="{{ $this->getId() }}.treeTable.record.{{ $recordKey }}.column.{{ $getName() }}.toggle-column.{{ $state ? 'true' : 'false' }}"
>
    <div
        x-data="{
            error: undefined,
    ...
    >
        ...
        <button
            ...
            x-bind:style="
                state
                    ? '{{ get_color_css_variables($onColor, shades: [600]) }}'
                    : '{{ get_color_css_variables($offColor, shades: [600]) }}'
            "
            @disabled($isDisabled())
            type="button"
