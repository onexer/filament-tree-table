...
<div
    ...
    {{
        $attributes
            ->merge([
                'wire:key' => "{$this->getId()}.treeTable.selection.indicator",
            ], escape: false)
            ->class([
...
>
    <div class="flex gap-x-3">
        ...
        <span
            x-text="
                window.pluralize(@js(__('filament-tree-table::table.selection_indicator.selected_count')), {{ $selectedRecordsPropertyName }}.length, {
    ...
    <div class="flex gap-x-3">
        <x-filament::link
            color="primary"
            :id="$this->getId() . '.treeTable.selection.indicator.record-count.' . $allSelectableRecordsCount"
            tag="button"
            :x-on:click="$selectAllRecordsAction"
            :x-show="$allSelectableRecordsCount . ' !== ' . $selectedRecordsPropertyName . '.length'"
        >
            {{ trans_choice('filament-tree-table::table.selection_indicator.actions.select_all.label', $allSelectableRecordsCount) }}
        </x-filament::link>
        ...
        >
            {{ __('filament-tree-table::table.selection_indicator.actions.deselect_all.label') }}
        </x-filament::link>
