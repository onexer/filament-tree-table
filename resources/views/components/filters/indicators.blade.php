...
>
    <div class="flex flex-col gap-x-3 gap-y-1 sm:flex-row">
        ...
        >
            {{ __('filament-tree-table::table.filters.indicator') }}
        </span>
        ...
        <div class="flex flex-wrap gap-1.5">
            @foreach ($indicators as $wireClickHandler => $label)
                <x-filament::badge>
                    ...
                    <x-slot
                        name="deleteButton"
                        :label="__('filament-tree-table::table.filters.actions.remove.label')"
                        wire:click="{{ $wireClickHandler }}"
                        wire:loading.attr="disabled"
                        wire:target="removeTreeTableFilter"
                    ></x-slot>
                </x-filament::badge>
            @endforeach
        </div>
    ...
    <x-filament::icon-button
        ...
        size="sm"
        :tooltip="__('filament-tree-table::table.filters.actions.remove_all.tooltip')"
        wire:click="removeTreeTableFilters"
        wire:target="removeTreeTableFilters,removeTreeTableFilter"
        class="-mx-1.5 -my-1"
    />
</div>
