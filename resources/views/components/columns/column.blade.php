...
@php
    use Filament\Support\Enums\Alignment;use Onexer\FilamentTreeTable\Actions\Action;
    ...
    @elseif (($action || $recordAction) && (! $isClickDisabled))
        @php
            if ($action instanceof Action) {
                $wireClickAction = "mountTreeTableAction('{$action->getName()}', '{$recordKey}')";
            } elseif ($action) {
                $wireClickAction = "callTableColumnAction('{$name}', '{$recordKey}')";
            } else {
                if ($this->getTreeTable()->getAction($recordAction)) {
                    $wireClickAction = "mountTreeTableAction('{$recordAction}', '{$recordKey}')";
                } else {
